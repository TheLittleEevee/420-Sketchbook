...
public class ChunkMeshController : MonoBehaviour
{
    ...
    {
        if((sides & 01) > 0) AddQuad(position + new Vector3(0, +0.5f, 0), Quaternion.identity, verts, tris, norms, uvs); //Top face (If one in digit one)
        if((sides & 02) > 0) AddQuad(position + new Vector3(0, -0.5f, 0), Quaternion.Euler(0, 0, 180), verts, tris, norms, uvs); //Bottom face
        if((sides & 04) > 0) AddQuad(position + new Vector3(+0.5f, 0, 0), Quaternion.Euler(0, 0, -90), verts, tris, norms, uvs); //Right face
        if((sides & 08) > 0) AddQuad(position + new Vector3(-0.5f, 0, 0), Quaternion.Euler(0, 0, 90), verts, tris, norms, uvs); //Left face
        if((sides & 16) > 0) AddQuad(position + new Vector3(0, 0, +0.5f), Quaternion.Euler(90, 0, 0), verts, tris, norms, uvs); //Front face
        if((sides & 32) > 0) AddQuad(position + new Vector3(0, 0, -0.5f), Quaternion.Euler(-90, 0, 0), verts, tris, norms, uvs); //Back face
    }
